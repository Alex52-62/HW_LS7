<template>
  <div class="calculator">
    ...
    <label
      ><input name="vk" type="checkbox" v-model="showvk" />Отобразить экранную
      клавиатуру</label
    >
    <div v-if="showvk">
      Виртуальная клавиатура
      <button name="vkI" v-for="btn in 10" :key="btn" v-bind:class="{[`active_${btn}`]: btn }" @click="inputNum(btn - 1)">
        {{ btn - 1 }}
      </button>
      <button class="erase" @click="eraseOne">E</button>
      <br /><br />
      <label
        ><input type="radio" name="oper1" value="1" v-model="operch" />Операнд
        1</label
      >
      <label
        ><input type="radio" name="oper2" value="2" v-model="operch" />Операнд
        2</label
      >
    </div>
  </div>
